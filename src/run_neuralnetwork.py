...
def run_nn(path_dict: dict,
    ...
    initial, cmd_rot_z, cmd_lin_x, cmd_lin_y = \
    ...
    for i_count in tqdm(range(0, len(torqueRR_Nm))):
        ...
        if nn_mode == "feedforward":
            ...
            temp[:, input_shape * (input_timesteps - 1) + output_shape] = cmd_rot_z[i_count]
            temp[:, input_shape * (input_timesteps - 1) + output_shape + 1] = cmd_lin_x[i_count]
            temp[:, input_shape * (input_timesteps - 1) + output_shape + 2] = cmd_lin_y[i_count]
            ...
            temp[0, input_timesteps - 1, output_shape] = cmd_rot_z[i_count]
            temp[0, input_timesteps - 1, output_shape + 1] = cmd_lin_x[i_count]
            temp[0, input_timesteps - 1, output_shape + 2] = cmd_lin_y[i_count]

        new_input = temp

    results[:, output_shape:input_shape] = data[startpoint:startpoint + len(cmd_rot_z) + input_timesteps,
